...
def safe_int_conversion(value: Any, default: int = 0) -> int:
    ...
    try:
        if isinstance(value, (int, float)):
            ...
            # Handle string conversion, remove whitespace
...
def retry_operation(operation, max_retries: int = 3, delay: float = 1.0, 
                   exception_types: tuple = (Exception,)) -> Any:
    """
...
def execute_robot_command(dashboard, command: str, *args, **kwargs) -> Tuple[bool, Any]:
    """
    Execute a robot command with error handling
    
    Args:
        dashboard: Robot dashboard instance
        command: Command name to execute
        *args: Positional arguments for command
        **kwargs: Keyword arguments for command
        
    Returns:
        (success, result): Command execution result
    """
    if not dashboard:
        return False, "Dashboard not available"
    
    try:
        if hasattr(dashboard, command):
            method = getattr(dashboard, command)
            result = method(*args, **kwargs)
            return True, result
        else:
            return False, f"Command '{command}' not available on dashboard"
    except Exception as e:
...
def wait_with_progress(duration: float, description: str = "Waiting", 
    ...
    Args:
        ...
        description: Description to show during wait
        step_size: Progress update interval
    """
