...
class DobotApiDashboard:
    """
    Unified dashboard API wrapper providing compatibility with existing code
    """
    
    def __init__(self, robot_ip: str = "192.168.1.6", port: int = 29999):
        self.robot_ip = robot_ip
        self.port = port
        self.robot_api = CoreRobotAPI(robot_ip)
    ...
    def connect(self) -> Tuple[bool, str]:
        ...
        if success:
            self._dashboard = self.robot_api.dashboard
    ...
    def is_connected(self) -> bool:
        """Check if connected"""
        return self._connected and self.robot_api.is_connected()
    
    def __getattr__(self, name):
        """Delegate all other calls to the actual dashboard API"""
        if self._dashboard and hasattr(self._dashboard, name):
...
class DobotApiFeedback:
    """
    Unified feedback API wrapper providing compatibility with existing code
    """
    ...
    def connect(self) -> Tuple[bool, str]:
        ...
        success, message = self.robot_api.connect()
        if success:
            self._feedback = self.robot_api.feedback
            self._connected = True
        return success, message
    ...
    def __getattr__(self, name):
        """Delegate all other calls to the actual feedback API"""
        if self._feedback and hasattr(self._feedback, name):
...
class ConnectionManager:
    """
    Connection manager for backward compatibility
    Provides centralized connection management for the robot
    """
    ...
    @classmethod
    def get_instance(cls, robot_ip: str = "192.168.1.6"):
        """Get or create singleton instance"""
        if not hasattr(cls, '_global_instance') or cls._global_instance is None:
            cls._global_instance = cls(robot_ip)
        return cls._global_instance
    
    def connect(self) -> Tuple[bool, str]:
        """Connect to robot"""
        return self.robot_api.connect()
    ...
    def get_dashboard(self):
        ...
        return self.robot_api.dashboard
    
    def get_feedback(self):
        """Get feedback instance"""
        return self.robot_api.feedback


class RobotStatusMonitor:
    """
    Robot status monitoring for backward compatibility
    """
    ...
    def check_status(self) -> Dict[str, Any]:
        """Check robot status"""
        return self.robot_api.get_robot_status()
    
    def check_alarms(self) -> Tuple[bool, List[str]]:
        """Check robot alarms"""
        return self.robot_api.check_robot_alarms()
    
    def clear_alarms(self) -> Tuple[bool, str]:
        """Clear robot alarms"""
        return self.robot_api.clear_robot_alarms()


def test_robot_connection(robot_ip: str = "192.168.1.6") -> Tuple[bool, str]:
    """
    Test robot connection for backward compatibility
    """
    api = CoreRobotAPI(robot_ip)
    return api.test_network_connectivity()


def check_robot_alarms(robot_ip: str = "192.168.1.6") -> Tuple[bool, List[str]]:
    """
    Check robot alarms for backward compatibility
    """
    api = CoreRobotAPI(robot_ip)
