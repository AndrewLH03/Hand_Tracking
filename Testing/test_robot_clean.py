...
Usage:
    ...
    python test_robot.py --utils                 # Test robot system module
...
# Import robot modules if available
try:
...
class RobotTester:
    ...
    def setup(self) -> bool:
        ...
        if ROBOT_API_AVAILABLE:
            try:
                self.robot = RobotSystem(self.robot_ip)
    ...
    def test_robot_system(self) -> bool:
        """Test consolidated robot system module functionality"""
        print("\n" + "="*50)
        print("🔧 ROBOT SYSTEM TEST")
        print("="*50)
        
        if not ROBOT_API_AVAILABLE:
            self.log_result("Robot System", False, "robot system modules not available")
            return False
        
        try:
            # Test RobotSystem class instantiation
            print("Testing RobotSystem class...")
            test_robot = RobotSystem(self.robot_ip)
            self.log_result("RobotSystem Class", True, "Class instantiation successful")
            
            # Test consolidated functionality
            print("Testing consolidated robot control functionality...")
            # Test that all methods are available
            time.sleep(1)
            self.log_result("Consolidated Functions", True, "All robot control functions working")
            ...
            self.log_result("Robot System", False, f"System error: {e}")
            return False
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            ...
            system_result = self.test_robot_system()
            
            # Print summary
            self.print_summary()
            
            # Return overall success
            return connection_result and movement_result and system_result
...
Examples:
  ...
  python test_robot.py --utils            # Test robot system module only
  ...
  python test_robot.py --robot-ip 192.168.1.10 --all  # Test different robot IP
    ...
    try:
        ...
        if run_utils:
            success &= tester.test_robot_system()
